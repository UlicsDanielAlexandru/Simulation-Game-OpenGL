...
{		
	...
	vec3 lightPosition = vec3(16.0f, 0.0f, -50.0f);

	lightDirN = normalize(lightPosition - fragPosEye.xyz);
	dist = length(lightPosition - fragPosEye.xyz);
	att = 1.0f / (constant + linear * dist + quadratic * (dist * dist));

	ambientPoint = att * ambientStrength * lightColor;
...
void main() 
{
	...
	vec3 color_final = color2 + color;

    float fogFactor = computeFog();
    vec4 fogColor = vec4(0.5f, 0.5f, 0.5f, 1.0f);
	fColor = fogColor * (1 - fogFactor) + vec4(color_final, 1.0f) * fogFactor;
